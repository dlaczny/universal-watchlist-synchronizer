...
- Remote-first poster grid with a focused detail panel for the selected item.
...
- Availability popup with `On Plex` and `Unavailable` checkboxes.
- `SharedPreferences` restore for the selected collection, sort mode, availability filters, and the last focused item where possible.
...
4. Open the availability popup from the filter icon and toggle `On Plex` and `Unavailable`. Confirm the grid updates for each combination.
5. Press Back while the popup is open. Confirm it closes without leaving the screen and focus returns to the filter icon.
6. Move focus to a poster, change collection, sort mode, and availability filters, then leave and relaunch the activity. Confirm the saved state and last focused item are restored where possible.
