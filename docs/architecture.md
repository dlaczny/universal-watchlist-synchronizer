...
2. Enrich movies with TMDB metadata (genres, watch providers, external IDs).
3. Fetch TMDB TV watchlist and enrich TV shows with TMDB details and external IDs.
...
- `POST /api/sync/tmdb/tv` — manual TMDB TV watchlist sync.
- `POST /api/sync/plex/movies` — manual Plex movie inventory sync and availability update.
- `POST /api/sync/all` — combined Letterboxd → TMDB movies → TMDB TV → Plex sync in order.
