...
- `Tmdb:AccountId` / `TMDB__AccountId`: TMDB account ID for TV watchlist sync.
- `Tmdb:SessionId` / `TMDB__SessionId`: TMDB session ID for TV watchlist sync.
- `Tmdb:Language`: defaults to `en-US`.

The backend starts without an access token so local read-only browsing still works. TMDB sync calls return a TMDB dependency error when the token is missing or invalid. The TV watchlist client additionally requires `AccountId` and `SessionId`.
...
Implemented TV watchlist sync:

- Source: TMDB account TV watchlist at `/account/{account_id}/watchlist/tv` with `session_id`, `language`, and `sort_by=created_at.desc`.
- Pages through all watchlist results.
- For each show, fetches `/tv/{series_id}?language={language}` for details and `/tv/{series_id}/external_ids` for IMDb/TVDB IDs.
- Builds poster URL with `ImageBaseUrl + /w500 + poster_path` and backdrop with `ImageBaseUrl + /w1280 + backdrop_path`.
- Stores TV records in `watchlist_items` with `MediaType.TvShow`, `Source.Tmdb`, and ID pattern `tv-tmdb-{tmdbId}`.
- Stores Mongo trace fields: `TmdbId`, `TvdbId`, `ImdbId`, `TmdbTitle`, `OriginalTitle`, `ReleaseDate`, `Genres`, `OriginalLanguage`, `TmdbVoteAverage`, `TmdbVoteCount`, `PosterPath`, `BackdropPath`, `TmdbMetadataUpdatedAt`, `TmdbMetadataStatus`.
- Removed TMDB TV watchlist items are deleted from the backend on the next sync.
- Manual trigger: `POST /api/sync/tmdb/tv`.
- Combined sync: included in `POST /api/sync/all` between TMDB movie enrichment and Plex movie sync.

Release status rules:

- Future `first_air_date` sets `ReleaseStatus.Unreleased` / `AvailabilityStatus.Unreleased`.
- Recognized TMDB statuses (`Ended`, `Returning Series`, `Canceled`, `In Production`) or a present first air date set `ReleaseStatus.Released` / `AvailabilityStatus.NotOnPlex`.
- Missing or unrecognized release state sets `ReleaseStatus.Unknown` / `AvailabilityStatus.UnknownMatch`.

Remaining gap for TV: Plex TV inventory sync and availability matching are not yet implemented. TV records show `NotOnPlex`, `Unreleased`, or `UnknownMatch` availability until Plex TV matching is added.
