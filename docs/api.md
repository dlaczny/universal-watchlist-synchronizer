...
TMDB TV watchlist sync is implemented, but the Sonarr-compatible TV export shape is not yet built. Version 1 reserves the endpoint and returns an empty array until the export shape is added.
...
Runs manual import of the configured TMDB account TV watchlist. The sync fetches all watchlist pages, enriches each show with TMDB details and external IDs, and persists normalized TV records to the database. Removed TMDB TV watchlist items are deleted from the backend store.
...
One metadata or dependency failure for one show does not stop the rest of the sync. When at least one item failed, the overall status is `partial`.
...
Runs Letterboxd movie sync, TMDB movie enrichment, TMDB TV watchlist sync, and Plex movie sync in order.
...
```json
{
  "status": "completed",
  "startedAt": "2026-06-05T12:00:00Z",
  "finishedAt": "2026-06-05T12:00:04Z",
  "letterboxd": {
    "status": "completed",
    "startedAt": "2026-06-05T12:00:00Z",
  ...
  "plexMovies": {
    "status": "completed",
    "startedAt": "2026-06-05T12:00:03Z",
    "finishedAt": "2026-06-05T12:00:04Z",
    "sectionsScanned": 1,
    "itemsFetched": 500,
    "itemsUpserted": 500,
